...
class GCONVCheb(nn.Module):
    ...
    def __init__(
        ...
        dense = nn.Sequential()
        dense.add_module(
            "lin0", nn.Linear(in_features=conv_dims[-1] * 2, out_features=dense_dims[0])
        )
        ...
        for i in range(1, len(dense_dims)):
            dense.add_module(
                f"lin{i}",
                nn.Linear(in_features=dense_dims[i - 1], out_features=dense_dims[i]),
            )
            dense.add_module(f"act{i}", self.act_fn)
            dense.add_module(f"dropout{i}", nn.Dropout(p=dropout_rate))
        dense.add_module(
            f"lin{len(dense_dims)}",
            nn.Linear(
                in_features=dense_dims[len(dense_dims) - 1], out_features=target_dim
            ),
        )
